---
layout: post
title: Compiling ArnoldC to JavaScript
---

At [NebraskaJS][nejs] I introduced my stupidest project yet: a compiler which converts [ArnoldC][arnoldc] programs into JavaScript, with full support for [Source Maps](/source-maps).
...
And while Write Once Run Anywhere might have been portable enough for 1998, my programs need to run on the Assembly Language of the Web.
...
You can then apply a *parser*, which is where the tokens get assembled into an Abstract Syntax Tree.
...
Jison was written by Mozilla's Zach Carter, and forms the spine of my compiler. It's a port of the C program Bison, which performs a similar role using a less fun language. But many of its [docs](http://dinosaur.compilertools.net/bison/bison_4.html#SEC7) might be useful to peruse.
...
This recursive pattern tends to show up a lot in language definitions.
...
So you've got a `BEGIN_MAIN` keyword, a set of statements, an `END_MAIN` keyword, and on either side a set of `methods` expressions (which are designed similarly to `statements`).
...
These videos from 2013 Front Trends were super helpful for me to wrap my 
